...
本项目早期由老师带着完成基础功能，后续由我在原有代码上持续完善和优化，包含功能开发、问题修复、页面调整、可靠性处理和测试工作。原始协作仓库保留了老师的基础开发提交；当前 `main` 分支用于展示包含本人后续维护成果的完整版本。

我重点参与和持续完善的内容包括：项目题库与面经搜索、个人中心页面拆分、按用户隔离的历史/收藏/点赞记录、学习时长记录、登录失效处理，以及录音权限、录制/播放生命周期和本地 RDB 元数据管理。

仓库提交历史用于反映真实协作关系；本文不将项目描述为本人从零独立开发。

题海方舟是一款面向求职与技能提升场景的 HarmonyOS NEXT 学习应用。应用以题库练习为核心，提供题目筛选与详情浏览、历史记录、收藏、点赞、打卡、学习时长统计、面经、项目资料、个人资料维护、英语单词和面试录音等功能。

> 本分支 `full` 保留完整应用。`main` 是基础框架，其余功能分支用于独立学习和重写，详见 [分支说明](docs/BRANCHES.md)。当前工程为单 `entry` 模块的 Stage 模型应用，主要面向手机设备。应用版本为 `8.3.0`；本机当前编译、目标与兼容基线为 HarmonyOS SDK `6.1.1(24)`，示例构建配置已同步。状态管理保持现有实现，本次不做 V2 迁移。
...
| 面经 | 按标题或作者搜索、最新/点赞最多/浏览最多排序、作者信息展示、详情阅读、点赞收藏分享和阅读埋点 |
...
建议在提交前至少完成：依赖同步、当前本地单元测试集、分支工具回归测试、Debug/Release HAP 构建、首页/登录/题目详情回归、麦克风权限拒绝与授权流程验证，以及浅色、深色和跟随系统主题检查。`ohosTest` 必须连接真机或模拟器执行；仅生成测试 HAP 不等于设备测试通过。
